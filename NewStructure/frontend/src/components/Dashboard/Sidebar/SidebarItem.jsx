  ...
  return (
    <ul className="flex flex-col p-2 pt-0 list-none space-y-3">
      {links.map(({ link, label, icon }) => (
        <li key={label}>
          <Link to={link} 
          className={` flex items-center p-4 pb-2 pt-2 gap-4 rounded-lg transition-all duration-300 
            ...
            <div className={`flex w-full ${isCollapsed ? "justify-right" : "justify-start"}`}>
              <span className="flex text-lg justify-center">{icon}</span>
